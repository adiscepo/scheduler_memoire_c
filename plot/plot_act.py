...
def create_gantt_chart(task_dict, missed_deadlines, max_tasks, ax):
...
def update(frame):
    ...
    create_gantt_chart(task_dict, missed_deadlines, max_tasks, ax)
    return []

# Lecture du fichier de logs (paramètres)
file_log = "output.log"
if len(sys.argv) >= 2:
    file_log = sys.argv[1]
...
ani = animation.FuncAnimation(fig, update, init_func=init, blit=True, interval=10)  # interval en ms
plt.show()
