...
void trigger_software_interrupt() {
    ...
    // irq_set_pending(SOFTWARE_INTERRUPT_IRQn);
}
...
size_t schedule() {
    ...
    for (size_t i = 0; i < MAX_PROCESSES; i++) {
        ...
        if (process->state == DEFINED && process->release_time <= NOW) {
            printf("R %d %d\n", i, NOW);
            process->state = READY;
        }
        ...
        if (process->state == READY && process->absolute_deadline < NOW) {
